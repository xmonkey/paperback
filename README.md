...
- **两种批改模式**：
  - 手动：键盘 1–4 评分，`Space` 切换答案
  - 拍照 OCR：手机拍默写卷上传，GLM 视觉识别 + 自动建议档位，人确认后写回
...
2. **session 页**：
   ...
   - 需要时打印「答案卷」对照
3. **批改**（二选一）：
   - **手动**：「开始批改」→ `Space` 显示答案 → `1`–`4` 评分（`Enter` 默认 **1 重来**，`Backspace` 上一张）
   - **拍照 OCR**：「📸 拍照批改」→ 手机拍照上传 → GLM 识别 + 建议档位 → 确认后写回（需先在 `/settings` 配置）
