...
**场景**：打印默写卷后、批改前，卡片可能在 Anki 被复习过（due 卡会出现在复习队列）。批改时 `answerCards` 会再评一次，覆盖第一次评分。

**决策：不设保护**（2026-06-24 评估）。

**理由**：默写（写出答案）比 Anki 复习（回想自评）更严格、信号更准。「Paperback 默写评分覆盖 Anki 回想评分」= 用更准的信号更新调度，是期望行为而非问题。若设保护（跳过默写评分），反而丢弃更准的信号、保留较不准的回想评分。

**代价**：被覆盖的卡 Anki 统计会记两次 review（reps+2），但用户确实评了两次（复习 + 默写），记两次合理。

**实测确认**：`answerCards` 对已复习（not due）的 review 卡会更新调度（reps+1、due 重算），返回 `[True]`，并非"静默失败"。
